package com.example.administrator.mybasewithtitle.proxy;
...
public class ThreadPoolProxy {
        ...
        if (mExecutor == null || mExecutor.isShutdown() || mExecutor.isTerminated()) {
            synchronized (ThreadPoolProxy.class) {
                if (mExecutor == null || mExecutor.isShutdown() || mExecutor.isTerminated()) {
                    ...
                    BlockingQueue<Runnable> workQueue = new LinkedBlockingQueue<>();//无限的任务队列
    ...
    /*
    提交任务和执行任务的区别,是否有返回值
            submit-->有-->Future , Future.cancel方法 可以用来关闭开启的线程，但是不一定线程是可以被关闭的，最好的家标记为，自己关闭
            execute-->没有
